...
const CODES = {
  java: [
    ...
    ['c_ctor', '        rear = -1;'],
    ['c_ctor', '    }'], ['', ''],
    ['c_deletefront', '    int deleteFront() {'],
    ['c_underflow', '        if (front == -1) {'],
    ['c_underflow', '            System.out.println("Deque Underflow");'],
    ['c_underflow', '            return -1;'],
    ['c_underflow', '        }'],
    ['c_capture', '        int value = arr[front];'],
    ['c_frontupdate', '        if (front == rear) {'],
    ['c_frontupdate', '            front = -1;'],
    ['c_frontupdate', '            rear = -1;'],
    ['c_frontupdate', '        } else {'],
    ['c_frontupdate', '            front = (front + 1) % capacity;'],
    ['c_frontupdate', '        }'],
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_deletefront', 'int deleteFront(struct Deque *dq) {'],
    ['c_underflow', '    if (dq->front == -1) {'],
    ['c_underflow', '        printf("Deque Underflow\\n");'],
    ['c_underflow', '        return -1;'],
    ['c_underflow', '    }'],
    ['c_capture', '    int value = dq->arr[dq->front];'],
    ['c_frontupdate', '    if (dq->front == dq->rear) {'],
    ['c_frontupdate', '        dq->front = -1;'],
    ['c_frontupdate', '        dq->rear = -1;'],
    ['c_frontupdate', '    } else {'],
    ['c_frontupdate', '        dq->front = (dq->front + 1) % dq->capacity;'],
    ['c_frontupdate', '    }'],
    ['c_return', '    return value;'],
    ['', '}'],
  ],
  ...
  python: [
    ...
    ['c_ctor', '        self.front = -1'],
    ['c_ctor', '        self.rear = -1'], ['', ''],
    ['c_deletefront', '    def delete_front(self):'],
    ['c_underflow', '        if self.front == -1:'],
    ['c_underflow', '            print("Deque Underflow")'],
    ['c_underflow', '            return -1'],
    ['c_capture', '        value = self.arr[self.front]'],
    ['c_frontupdate', '        if self.front == self.rear:'],
    ['c_frontupdate', '            self.front = -1'],
    ['c_frontupdate', '            self.rear = -1'],
    ['c_frontupdate', '        else:'],
    ['c_frontupdate', '            self.front = (self.front + 1) % self.capacity'],
    ['c_return', '        return value'],
  ],
  javascript: [
    ['', 'class Deque {'],
    ...
    ['c_ctor', '    this.rear = -1;'],
    ['c_ctor', '  }'], ['', ''],
    ['c_deletefront', '  deleteFront() {'],
    ['c_underflow', '    if (this.front === -1) {'],
    ['c_underflow', '      console.log("Deque Underflow");'],
    ['c_underflow', '      return -1;'],
    ['c_underflow', '    }'],
    ['c_capture', '    const value = this.arr[this.front];'],
    ['c_frontupdate', '    if (this.front === this.rear) {'],
    ['c_frontupdate', '      this.front = -1;'],
    ['c_frontupdate', '      this.rear = -1;'],
    ['c_frontupdate', '    } else {'],
    ['c_frontupdate', '      this.front = (this.front + 1) % this.capacity;'],
    ['c_frontupdate', '    }'],
    ['c_return', '    return value;'],
    ['', '  }'], ['', '}'],
  ],
...
//   'del'    - the front slot currently being removed (red pulse), before
//              it is cleared
function buildSteps(capacity, frontIdx, values) {
  const steps = [];
  const phaseStarts = { deleteFront: 0 };
  ...
  // --- value = arr[front]: capture the value before it is removed.
  const targetIdx = front;
  removed = valAt[targetIdx];
  stateAt[targetIdx] = 'del';
  pushStep({
    badge: 'value = arr[front] \u2192 capturing ' + removed + ' from index ' + targetIdx + ' before removing it',
    code: 'c_capture',
  ...
  // --- clear the slot and move front (or reset both pointers if this was
  ...
  if (front === rear) {
    ...
    front = (front + 1) % capacity;
    const wrapped = front < oldFront;
    pushStep({
      badge: wrapped
        ? 'front = (front + 1) % capacity \u2192 wraps around from index ' + oldFront + ' to index ' + front
        : 'front = (front + 1) % capacity \u2192 front moves to index ' + front,
      code: 'c_frontupdate',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(removed), true]])],
    });
  }
...
const stepsData = reactive(buildSteps(5, 4, [10, 20]));
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                ...
                <tr><td>Normal deleteFront</td><td>O(1)</td><td>direct read from <code>arr[front]</code> followed by one modulo increment &mdash; no traversal or shifting</td></tr>
                <tr><td>Wrap-around deletion</td><td>O(1)</td><td>the modulo operation folds index <code>capacity</code> back to <code>0</code> in constant time</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used &mdash; the slot is simply marked free</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>deleteFront</code> never shifts any other elements to fill the gap. It only reads
              <code>arr[front]</code>, then advances <code>front</code> with <code>(front + 1) % capacity</code> so the
              pointer wraps from index <code>capacity - 1</code> back to <code>0</code> instead of running off the array.
              The special case <code>front == rear</code> means only one element remained, so after removing it both
              pointers are reset to <b>-1</b> to correctly represent an empty deque &mdash; without this reset the deque
              would incorrectly believe a stale slot is still occupied.
            </p>
            <h3>Advantages of Deque</h3>
            <p>Deletion is O(1) from <b>both</b> ends, unlike a simple array where removing the front element requires
              shifting every remaining element left. The fixed-size circular array is cache-friendly and needs no
              per-element pointer overhead, unlike a linked-list based deque.</p>
            <h3>Limitations</h3>
            ...
            <p>A <b>Simple Queue</b> only allows removal at the front and insertion at the rear, and once emptied its
              pointers are typically just reset rather than wrapped. A <b>Circular Queue</b> wraps both pointers with
              modulo arithmetic but still restricts removal to the front only. A <b>Deque (Double-Ended Queue)</b> goes
              further: using the same circular indexing, it allows removal <b>and</b> insertion at <b>both</b> the front
              and the rear, making it strictly more flexible than either a simple or circular queue.</p>
            <h3>Key Observations</h3>
            <p>Deleting the last remaining element (<code>front == rear</code>) is a special case that resets both
              <code>front</code> and <code>rear</code> to <b>-1</b>, rather than just advancing <code>front</code> &mdash;
              this is what correctly signals an empty deque to future operations. Every other deleteFront only moves
              <code>front</code> forward circularly, leaving <code>rear</code> untouched. The current size is always
              <code>((rear - front + capacity) % capacity) + 1</code> (or 0 when empty), which correctly accounts for
              wrap-around.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty deque:</b> deleteFront must detect <code>front == -1</code> and report Deque Underflow without
              touching the array. <b>Single-element deque:</b> after capturing the value, both <code>front</code> and
              <code>rear</code> reset to -1. <b>Multiple-element deque:</b> only <code>front</code> advances circularly;
              <code>rear</code> and every other element stay exactly where they are. <b>Wrap-around deletion:</b> when
              <code>front</code> is at index <code>capacity - 1</code>, the next deleteFront wraps it to <code>0</code>
              instead of running past the end of the array.</p>
          </div>
        </div>
      </div>
    </div>
