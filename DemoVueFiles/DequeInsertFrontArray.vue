...
const CODES = {
  java: [
    ...
    ['c_ctor', '        rear = -1;'],
    ['c_ctor', '    }'], ['', ''],
    ['c_insertfront', '    void insertFront(int value) {'],
    ['c_overflow', '        if ((rear + 1) % capacity == front) {'],
    ['c_overflow', '            System.out.println("Deque Overflow");'],
    ['c_overflow', '            return;'],
    ['c_overflow', '        }'],
    ['c_frontset', '        if (front == -1) {'],
    ['c_frontset', '            front = 0;'],
    ['c_frontset', '            rear = 0;'],
    ['c_frontset', '        } else {'],
    ['c_frontset', '            front = (front - 1 + capacity) % capacity;'],
    ['c_frontset', '        }'],
    ['c_write', '        arr[front] = value;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ['', '#define MAX 100'], ['', ''],
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_insertfront', 'void insertFront(struct Deque *dq, int value) {'],
    ['c_overflow', '    if ((dq->rear + 1) % dq->capacity == dq->front) {'],
    ['c_overflow', '        printf("Deque Overflow\\n");'],
    ['c_overflow', '        return;'],
    ['c_overflow', '    }'],
    ['c_frontset', '    if (dq->front == -1) {'],
    ['c_frontset', '        dq->front = 0;'],
    ['c_frontset', '        dq->rear = 0;'],
    ['c_frontset', '    } else {'],
    ['c_frontset', '        dq->front = (dq->front - 1 + dq->capacity) % dq->capacity;'],
    ['c_frontset', '    }'],
    ['c_write', '    dq->arr[dq->front] = value;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Deque {'],
  ...
  python: [
    ...
    ['c_ctor', '        self.front = -1'],
    ['c_ctor', '        self.rear = -1'], ['', ''],
    ['c_insertfront', '    def insert_front(self, value):'],
    ['c_overflow', '        if (self.rear + 1) % self.capacity == self.front:'],
    ['c_overflow', '            print("Deque Overflow")'],
    ['c_overflow', '            return'],
    ['c_frontset', '        if self.front == -1:'],
    ['c_frontset', '            self.front = 0'],
    ['c_frontset', '            self.rear = 0'],
    ['c_frontset', '        else:'],
    ['c_frontset', '            self.front = (self.front - 1 + self.capacity) % self.capacity'],
    ['c_write', '        self.arr[self.front] = value'],
  ],
  javascript: [
    ['', 'class Deque {'],
    ...
    ['c_ctor', '    this.rear = -1;'],
    ['c_ctor', '  }'], ['', ''],
    ['c_insertfront', '  insertFront(value) {'],
    ['c_overflow', '    if ((this.rear + 1) % this.capacity === this.front) {'],
    ['c_overflow', '      console.log("Deque Overflow");'],
    ['c_overflow', '      return;'],
    ['c_overflow', '    }'],
    ['c_frontset', '    if (this.front === -1) {'],
    ['c_frontset', '      this.front = 0;'],
    ['c_frontset', '      this.rear = 0;'],
    ['c_frontset', '    } else {'],
    ['c_frontset', '      this.front = (this.front - 1 + this.capacity) % this.capacity;'],
    ['c_frontset', '    }'],
    ['c_write', '    this.arr[this.front] = value;'],
    ['', '  }'], ['', '}'],
  ],
};
...
function buildSteps(capacity, frontIdx, values, insertValue) {
  ...
  const isFull = () => (capacity === 0 ? true : (rear + 1) % capacity === front);
  ...
  if (front === -1) {
    ...
    rear = 0;
    pushStep({
      badge: 'front == -1 \u2192 deque was empty, front = rear = 0',
      code: 'c_frontset',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(insertValue), true]])],
    });
  } else {
    ...
    pushStep({
      badge: wrapped
        ? 'front = (front - 1 + capacity) % capacity \u2192 wraps around from index ' + oldFront + ' to index ' + front
        : 'front = (front - 1 + capacity) % capacity \u2192 front moves to index ' + front,
      code: 'c_frontset',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(insertValue), true]])],
    });
  }

  // --- the pointer has landed on its target slot; nothing written yet.
  stateAt[front] = 'cur';
  pushStep({
    badge: 'front now points to index ' + front + ' (still empty)',
    code: 'c_frontset',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(insertValue), true]])],
  });

  // --- arr[front] = value: the value is actually written into that slot.
  valAt[front] = insertValue;
  stateAt[front] = 'new';
  pushStep({
    badge: 'arr[front] = ' + insertValue + ' \u2192 index ' + front + ' now holds ' + insertValue,
    code: 'c_write',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(insertValue), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>front's target slot</span>
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                ...
                <tr><td>Normal insertFront</td><td>O(1)</td><td>direct write to <code>arr[front]</code> after one modulo decrement &mdash; no traversal or shifting</td></tr>
                <tr><td>Wrap-around insertion</td><td>O(1)</td><td>the modulo operation folds index -1 back to <code>capacity - 1</code> in constant time</td></tr>
            ...
            <p class="ll-note">
              Key observation: because <code>front</code> is decremented with <code>(front - 1 + capacity) % capacity</code>
              instead of a plain <code>front - 1</code>, the pointer wraps from index 0 straight to
              <code>capacity - 1</code> instead of going negative. This circular indexing is what lets a Deque reuse the
              slots freed up at the rear end, and is the key difference from a simple linear array shift, which would need
              O(n) time to make room at the front.
            </p>
            <h3>Advantages of Deque</h3>
            <p>Insertion and deletion are O(1) from <b>both</b> ends, unlike a simple array where inserting at the front
              requires shifting every existing element. The fixed-size circular array is cache-friendly and needs no
            ...
            <p>A <b>Simple Queue</b> only allows insertion at the rear and removal at the front, and <code>rear</code> never
              wraps &mdash; once it hits <code>capacity - 1</code> it stops for good even if front slots are free. A
              <b>Circular Queue</b> fixes that by wrapping both pointers with modulo arithmetic, but still restricts
            ...
            <p><code>front</code> and <code>rear</code> both start at <b>-1</b> for an empty deque. The very first
              <code>insertFront</code> (or <code>insertRear</code>) sets both <code>front</code> and <code>rear</code> to
              <b>0</b>. Every insertFront after that only moves <code>front</code> backwards circularly, leaving
              <code>rear</code> untouched. The current size is always <code>((rear - front + capacity) % capacity) + 1</code>
              (or 0 when the deque is empty), which correctly accounts for wrap-around.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty deque:</b> the first insertFront sets <code>front</code> and <code>rear</code> from -1 to 0 in
              addition to placing the value. <b>First insertion:</b> identical to the empty-deque case above. <b>Multiple
              insertions:</b> each subsequent insertFront only decrements <code>front</code> circularly and writes the new
              value, leaving <code>rear</code> unchanged. <b>Wrap-around insertion:</b> when <code>front</code> is at index
              0, the next insertFront wraps it to <code>capacity - 1</code> instead of going to -1. <b>Full deque:</b> when
              <code>(rear + 1) % capacity == front</code>, insertFront must detect overflow and leave the deque completely
              unchanged &mdash; no write happens and neither pointer moves.</p>
          </div>
        </div>
      </div>
    </div>
