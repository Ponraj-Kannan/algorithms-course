...
const CODES = {
  java: [
    ...
    ['c_ctor', '    }'], ['', ''],
    ['c_size', '    int size() {'],
    ['c_emptycase', '        if (front == -1) {'],
    ['c_emptycase', '            return 0;'],
    ['c_emptycase', '        }'],
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_size', 'int size(struct Deque *dq) {'],
    ['c_emptycase', '    if (dq->front == -1) {'],
    ['c_emptycase', '        return 0;'],
    ['c_emptycase', '    }'],
    ['c_formula', '    return ((dq->rear - dq->front + dq->capacity) % dq->capacity) + 1;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Deque {'],
...
//   'peek'   - an occupied slot currently being counted towards size
//              (purple highlight); no value is removed and no pointer moves
function buildSteps(capacity, frontIdx, values) {
  ...
  const fnLabel = 'size()';
  const trueSize = () => (front === -1 ? 0 : ((rear - front + capacity) % capacity) + 1);
  let result = null;
  ...
  function pushStep(opts) {
    steps.push({
      cells: cells(),
      front, rear, capacity,
      size: result === null ? trueSize() : result,
      result,
      badge: opts.badge,
      code: opts.code,
  ...
  pushStep({
    badge: 'Initial deque \u2014 capacity ' + capacity + ' \u2014 ' + fnLabel + ' called',
    code: 'c_size',
  ...
  if (front === -1) {
    result = 0;
    pushStep({
      badge: 'front == -1 \u2192 deque is empty, size = 0',
      code: 'c_emptycase',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['result', '0', true]])],
    });
  } else {
    // highlight every occupied cell from front to rear (circularly) while
    // the size formula is being evaluated
    for (let i = 0; i < n; i++) {
      const idx = (front + i) % capacity;
      if (stateAt[idx]) stateAt[idx] = 'peek';
    }
    result = trueSize();
    pushStep({
      badge: 'size = ((rear - front + capacity) % capacity) + 1 \u2192 ((' + rear + ' - ' + front + ' + ' + capacity + ') % ' + capacity + ') + 1 = ' + result,
      code: 'c_formula',
    ...
    for (let i = 0; i < n; i++) {
      const idx = (front + i) % capacity;
      if (stateAt[idx]) stateAt[idx] = 'normal';
    }
  }

  pushStep({
    badge: fnLabel + ' returned ' + result + ' out of a maximum of ' + capacity,
    code: 'c_size',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div class="ll-ptr-chip">size() = <b class="ll-c-purple">{{ s.result === null ? '\u2014' : s.result }}</b></div>
            </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-peek"></span>cell counted towards size</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty deque</td><td>O(1)</td><td>a single comparison short-circuits straight to 0</td></tr>
                <tr><td>Any non-empty deque</td><td>O(1)</td><td>one subtraction, one modulo, and one addition &mdash; no traversal of the array at all</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used; the deque is left completely unchanged</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>size</code> never needs to walk the array counting occupied cells one by one. The
              formula <code>((rear - front + capacity) % capacity) + 1</code> works because <code>rear - front</code>
              gives the distance from front to rear when there is no wrap-around, and adding <code>capacity</code> before
              taking the modulo correctly "unwraps" that distance when <code>rear</code> is numerically smaller than
              <code>front</code> (i.e. the occupied region crosses the end of the array). The <code>+ 1</code> accounts
              for the fact that both <code>front</code> and <code>rear</code> themselves are occupied slots being
              counted.
            </p>
            <h3>Advantages of Deque</h3>
            <p>Knowing the current size in O(1) &mdash; without scanning the array &mdash; lets other operations (like
              isFull, or application code deciding whether to insert more elements) stay efficient even as the deque
              grows large.</p>
            <h3>Limitations</h3>
            <p>The formula relies entirely on <code>front</code> and <code>rear</code> being kept perfectly consistent by
              every insert and delete operation. If either pointer were ever updated incorrectly, <code>size</code> would
              silently report the wrong value instead of detecting the corruption.</p>
            <h3>Difference between Queue, Circular Queue, and Deque</h3>
            <p>A <b>Simple Queue</b> that never wraps can compute its size as a plain <code>rear - front + 1</code>. A
              <b>Circular Queue</b> needs the modulo-based formula shown here to handle wrap-around, since <code>rear</code>
              can be numerically smaller than <code>front</code>. A <b>Deque</b> uses the exact same modulo-based formula
              as a Circular Queue &mdash; the size calculation itself does not change just because insertions and
              deletions are now allowed at both ends.</p>
            <h3>Key Observations</h3>
            <p>The size formula and the <code>isEmpty</code> check are related but not interchangeable: size must
              special-case <code>front == -1</code> and return 0 directly, because plugging <code>front = rear = -1</code>
              into <code>((rear - front + capacity) % capacity) + 1</code> would incorrectly evaluate to 1 instead of 0.
              For every non-empty state, though, the same formula correctly reports anywhere from 1 up to
              <code>capacity</code> elements.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty deque:</b> must be special-cased to return 0 rather than relying on the general formula.
              <b>Single-element deque:</b> <code>front == rear</code>, so the formula correctly reduces to 1.
              <b>Non-wrapped deque:</b> <code>front &lt; rear</code>, so the formula behaves just like a simple
              <code>rear - front + 1</code>. <b>Wrapped deque:</b> <code>rear &lt; front</code>, so adding
              <code>capacity</code> before the modulo correctly accounts for the elements that wrap past the end of the
              array. <b>Full deque:</b> the formula correctly evaluates to exactly <code>capacity</code>.</p>
          </div>
        </div>
      </div>
    </div>
