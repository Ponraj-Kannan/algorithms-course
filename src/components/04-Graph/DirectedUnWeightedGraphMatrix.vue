...
const PSEUDOCODE = [
  ...
  '        adjMatrix[source][destination] = 1  // directed edge',
  '',
  'main():',
  '    read vertices',
...
function buildSteps(numV, edgeList) {
  ...
  for (let i = 0; i < edgeList.length; i++) {
    ...
    if (isValid) {
      ...
      if (existingIdx < 0) processedEdges.push({ u, v });

      steps.push({
        badge: `adjMatrix[${u}][${v}] = 1; → Set matrix cell [${u}][${v}] = 1 (directed edge from ${u} → ${v})`,
        code: 'c_set_uv',
  ...
  steps.push({
    badge: `main() finished: All ${E} directed edges created and stored in Graph.`,
    code: 'c_main_done',
                ...
                <div v-else class="ll-info-scroll">
                  ...
                  <table class="ll-complexity-table">
                    ...
                    <tbody>
                      ...
                      <tr><td>Add Edge <code>addEdge(u, v)</code></td><td>O(1)</td><td>Direct indexing access <code>adj[u][v] = 1</code>.</td></tr>
                      <tr><td>Query Edge <code>getEdge(u, v)</code></td><td>O(1)</td><td>Direct array lookup <code>return adj[u][v]</code>.</td></tr>
                      <tr><td>Space Complexity</td><td>O(V&sup2;)</td><td>Stores a V x V array in memory.</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    Key feature: <b>2D Adjacency Matrix</b> for directed unweighted graphs stores `1` for directed edge `u → v`. Allows instant <code>O(1)</code> lookup.
                  </p>
                </div>
              </div>
            </div>
