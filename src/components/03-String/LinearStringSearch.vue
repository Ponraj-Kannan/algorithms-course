...
defineProps({
  topic: { type: String, default: 'Linear String Search' },
  subTopic: { type: String, default: 'Character Search in String' }
});

const CODES = {
  java: [
    ['', 'class LinearStringSearch {'],
    ['c_entry', '    int search(String text, char target) {'],
    ['c_n', '        int n = text.length();'],
    ['c_loop', '        for (int i = 0; i < n; i++) {'],
    ['c_check', '            if (text.charAt(i) == target) {'],
    ['c_found', '                return i;'],
    ['', '            }'],
    ['', '        }'],
    ['c_not_found', '        return -1;'],
    ['c_done', '    }'],
    ['', '}']
  ],
  c: [
    ['c_entry', 'int search(const char* text, char target) {'],
    ['c_n', '    int n = strlen(text);'],
    ['c_loop', '    for (int i = 0; i < n; i++) {'],
    ['c_check', '        if (text[i] == target) {'],
    ['c_found', '            return i;'],
    ['', '        }'],
    ['', '    }'],
    ['c_not_found', '    return -1;'],
  ...
  python: [
    ['', 'class LinearStringSearch:'],
    ['c_entry', '    def search(self, text, target):'],
    ['c_n', '        n = len(text)'],
    ['c_loop', '        for i in range(n):'],
    ['c_check', '            if text[i] == target:'],
    ['c_found', '                return i'],
    ['c_not_found', '        return -1'],
    ['c_done', '']
  ...
  javascript: [
    ['', 'class LinearStringSearch {'],
    ['c_entry', '  search(text, target) {'],
    ['c_n', '    const n = text.length;'],
    ['c_loop', '    for (let i = 0; i < n; i++) {'],
    ['c_check', '      if (text[i] === target) {'],
    ['c_found', '        return i;'],
    ['', '      }'],
    ['', '    }'],
...
const PSEUDOCODE = [
  ...
  '    for i = 0 to n - 1:',
  '        if text[i] == target:',
  '            return i   // Target character found',
  '    return -1          // Target character not found',
];

function frame(title, rows) { return { title, rows }; }

function buildSteps(textStr, targetChar) {
  const steps = [];
  const chars = textStr.split('');
  const n = chars.length;
  const target = String(targetChar).charAt(0) || '';
  const fnLabel = 'search(text, target)';

  function currentCellStates(currIdx, matchIdx = -1) {
    return chars.map((val, idx) => {
      if (idx === matchIdx) return { idx, val, state: 'found' };
      if (idx === currIdx) return { idx, val, state: 'cur' };
      if (idx < currIdx) return { idx, val, state: 'checked' };
  ...
  // 1. Entry
  steps.push({
    badge: `search called with text="${textStr}" and target='${target}'`,
    code: 'c_entry',
    vars: [
      frame('main()', []),
      frame(fnLabel, [['text', `"${textStr}"`], ['target', `'${target}'`]])
    ],
    cells: currentCellStates(-1),
    i: -1, n, comparisons: 0, found: false
  });

  if (n === 0) {
    steps.push({
      badge: 'Text string is empty (n = 0) → return -1',
      code: 'c_not_found',
      vars: [frame('main()', []), frame(fnLabel, [['n', '0'], ['return', '-1']])],
      cells: [],
      i: -1, n: 0, comparisons: 0, found: false
    });
    return steps;
  }
  ...
  // 2. n = text.length()
  steps.push({
    badge: `int n = text.length() → n set to ${n}`,
    code: 'c_n',
    vars: [
      frame('main()', []),
      frame(fnLabel, [['n', String(n)], ['text', `"${textStr}"`], ['target', `'${target}'`]])
    ],
    cells: currentCellStates(-1),
    i: -1, n, comparisons: 0, found: false
  });
  ...
  for (let i = 0; i < n; i++) {
    steps.push({
      badge: `for (i = ${i}; i < ${n}; i++) → inspecting character at index ${i}`,
      code: 'c_loop',
      vars: [
        frame('main()', []),
        frame(fnLabel, [['n', String(n)], ['i', String(i)], ['text[i]', `'${chars[i]}'`]])
      ],
      cells: currentCellStates(i),
      i, n, comparisons, found: false
    });

    comparisons++;
    const isMatch = chars[i] === target;
    steps.push({
      badge: `if (text[${i}] == '${target}') → checking '${chars[i]}' == '${target}' → ${isMatch ? 'MATCH FOUND!' : 'NO MATCH'}`,
      code: 'c_check',
      vars: [
        frame('main()', []),
        frame(fnLabel, [['i', String(i)], ['text[i]', `'${chars[i]}'`], ['target', `'${target}'`]])
      ],
      cells: isMatch ? currentCellStates(-1, i) : currentCellStates(i),
      i, n, comparisons, found: isMatch
    });

    if (isMatch) {
      foundIdx = i;
      steps.push({
        badge: `return ${i} → Target character '${target}' found at index ${i} after ${comparisons} comparisons`,
        code: 'c_found',
        vars: [
          frame('main()', []),
          frame(fnLabel, [['return', String(i)], ['status', 'FOUND']])
        ],
        cells: currentCellStates(-1, i),
        i, n, comparisons, found: true
      });
      break;
    }
  }
  ...
  if (foundIdx === -1) {
    steps.push({
      badge: `return -1 → Target character '${target}' not found in string "${textStr}" after ${comparisons} comparisons`,
      code: 'c_not_found',
      vars: [
        frame('main()', []),
        frame(fnLabel, [['return', '-1'], ['status', 'NOT_FOUND']])
      ],
      cells: chars.map((v, idx) => ({ idx, val: v, state: 'checked' })),
      i: n, n, comparisons, found: false
    });
  }

  steps.push({
    badge: foundIdx !== -1 ? `Search Complete: Target '${target}' found at index ${foundIdx}` : `Search Complete: Target '${target}' not found`,
    code: 'c_done',
    vars: [
      frame('main()', []),
      frame(fnLabel, [['result', String(foundIdx)]])
    ],
    cells: chars.map((v, idx) => ({ idx, val: v, state: idx === foundIdx ? 'found' : 'checked' })),
    i: foundIdx, n, comparisons, found: foundIdx !== -1
  });

  return steps;
}

const inpText = ref('ANTIGRAVITY');
const inpTarget = ref('G');
const lang = ref('java');
const speed = ref(650);
const si = ref(0);
...
const stepsData = reactive({ steps: buildSteps('ANTIGRAVITY', 'G') });
...
function applyInput() {
  const textStr = String(inpText.value || '');
  const targetChar = String(inpTarget.value || '').charAt(0);
  if (!textStr.length) {
    alert('Please enter a non-empty text string.');
    return;
  }

  playing.value = false;
  stepsData.steps = buildSteps(textStr, targetChar);
  si.value = 0;
}
...
function cellLabel(cell) {
  if (s.value.i === cell.idx && !s.value.found) return 'i';
  if (cell.state === 'found') return 'MATCH';
  return '';
}
      ...
      <div class="row-main">
        <div class="ll-root">
          ...
          <div class="ll-toolbar">
            <label>Text</label>
            <input type="text" v-model="inpText" style="width: 150px;" class="ll-text-input" placeholder="e.g. ANTIGRAVITY" />
            
            <label>Target</label>
            <input type="text" v-model="inpTarget" style="width: 50px; text-align: center;" class="ll-text-input" maxlength="1" placeholder="G" />
          ...
          <div class="ll-main" ref="mainRef">
            ...
            <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
              <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
                <div class="ll-perm-area">
                  <div class="ll-ptrs">
                    ...
                    <div class="ll-ptr-chip">Target = <b class="ll-c-amber">'{{ inpTarget }}'</b></div>
                      ...
                      <div class="ll-arr-cell-wrap">
                        ...
                        <div
                          ...
                          :class="{
                            ...
                            'll-box-checked': cell.state === 'checked'
                          }"
                        >
                          {{ cell.val }}
                        </div>
              ...
              <!-- Color Legend -->
              <div class="ll-legend">
                <span class="ll-leg"><span class="ll-legdot ll-legdot-existing"></span>Unsearched</span>
                <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>Inspecting Character (i)</span>
                <span class="ll-leg"><span class="ll-legdot ll-legdot-checked"></span>Checked (No Match)</span>
              ...
              <div class="ll-code-panel">
                ...
                <div v-else class="ll-info-scroll">
                  ...
                  <table class="ll-complexity-table">
                    ...
                    <tbody>
                      <tr><td>Best Case</td><td>O(1)</td><td>Target character is found at the first position (index 0).</td></tr>
                      <tr><td>Average Case</td><td>O(n)</td><td>Scans through on average n/2 characters.</td></tr>
                      <tr><td>Worst Case</td><td>O(n)</td><td>Target is at index n-1 or does not exist in text string.</td></tr>
                      <tr><td>Space</td><td>O(1)</td><td>Requires constant auxiliary memory.</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    Key observation: Linear string search checks each character sequentially from index <code>0</code> to <code>n - 1</code> until a match is found.
                  </p>
                </div>
              </div>
            </div>
...
.ll-text-input { width: 150px; padding: 5px 10px; }
...
.ll-box-checked { border-color: #cbd5e1 !important; background: #f1f5f9 !important; color: #64748b !important; }
.ll-box-found { border-color: #10b981 !important; background: #dcfce7 !important; color: #065f46 !important; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.25) !important; transform: scale(1.05); }
...
.ll-legdot-checked { background: #f1f5f9; border: 1.5px solid #cbd5e1; }
