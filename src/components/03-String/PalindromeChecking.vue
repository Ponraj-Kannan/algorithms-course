...
defineProps({
  topic: { type: String, default: 'Palindrome Checking Algorithm' },
  subTopic: { type: String, default: 'Two-Pointer Character Comparison' }
});

const CODES = {
  java: [
    ['', 'class PalindromeChecking {'],
    ['c_entry', '    boolean isPalindrome(String s) {'],
    ['c_left', '        int left = 0;'],
    ['c_right', '        int right = s.length() - 1;'],
    ['c_while', '        while (left < right) {'],
    ['c_check', '            if (s.charAt(left) != s.charAt(right)) {'],
    ['c_mismatch', '                return false;'],
    ['', '            }'],
    ['c_inc', '            left++; right--;'],
    ['', '        }'],
  ...
  c: [
    ['c_entry', 'bool isPalindrome(const char* s) {'],
    ['c_left', '    int left = 0;'],
    ['c_right', '    int right = strlen(s) - 1;'],
    ['c_while', '    while (left < right) {'],
    ['c_check', '        if (s[left] != s[right]) {'],
    ['c_mismatch', '            return false;'],
    ['', '        }'],
    ['c_inc', '        left++; right--;'],
    ['', '    }'],
  ...
  cpp: [
    ['', 'class PalindromeChecking {'],
    ['', 'public:'],
    ['c_entry', '    bool isPalindrome(string s) {'],
    ['c_left', '        int left = 0;'],
    ['c_right', '        int right = s.length() - 1;'],
    ['c_while', '        while (left < right) {'],
    ['c_check', '            if (s[left] != s[right]) {'],
  ...
  python: [
    ['', 'class PalindromeChecking:'],
    ['c_entry', '    def is_palindrome(self, s):'],
    ['c_left', '        left = 0'],
    ['c_right', '        right = len(s) - 1'],
    ['c_while', '        while left < right:'],
    ['c_check', '            if s[left] != s[right]:'],
    ['c_mismatch', '                return False'],
  ...
  javascript: [
    ['', 'class PalindromeChecking {'],
    ['c_entry', '  isPalindrome(s) {'],
    ['c_left', '    let left = 0, right = s.length - 1;'],
    ['c_while', '    while (left < right) {'],
    ['c_check', '      if (s[left] !== s[right]) {'],
    ['c_mismatch', '        return false;'],
    ['', '      }'],
...
const PSEUDOCODE = [
  ...
  '        left = left + 1',
  '        right = right - 1',
  '    return true            // All character pairs match',
];
...
function buildSteps(initialStr) {
  ...
  const fnLabel = 'isPalindrome(s)';
  const matchedSet = new Set();
  let mismatchPair = [];

  function currentCellStates(leftIdx, rightIdx, isChecking = false) {
    return chars.map((val, idx) => {
      if (mismatchPair.includes(idx)) return { idx, val, state: 'mismatch' };
      if (isChecking && (idx === leftIdx || idx === rightIdx)) return { idx, val, state: 'cur' };
      if (idx === leftIdx) return { idx, val, state: 'left_ptr' };
      if (idx === rightIdx) return { idx, val, state: 'right_ptr' };
      if (matchedSet.has(idx)) return { idx, val, state: 'matched' };
  ...
  // 1. Entry
  steps.push({
    badge: `isPalindrome called with string "${initialStr}"`,
    code: 'c_entry',
    vars: [
      frame('main()', []),
      frame(fnLabel, [['s', `"${initialStr}"`]])
    ],
    cells: currentCellStates(-1, -1),
    left: -1, right: -1, matches: 0, n, result: null
  });

  if (n === 0) {
    steps.push({
      badge: 'String is empty (n = 0) → return true',
      code: 'c_return_true',
      vars: [frame('main()', []), frame(fnLabel, [['return', 'true']])],
      cells: [],
      left: -1, right: -1, matches: 0, n: 0, result: true
    });
    return steps;
  }
  ...
  steps.push({
    ...
    vars: [
      frame('main()', []),
      frame(fnLabel, [['left', '0'], ['s[left]', `'${chars[0]}'`]])
    ],
    cells: currentCellStates(left, -1),
    left, right: -1, matches: 0, n, result: null
  });

  // 3. right = n - 1
  let right = n - 1;
  steps.push({
    badge: `int right = s.length - 1 → right pointer initialized to index ${right} ('${chars[right]}')`,
    code: 'c_right',
    vars: [
      frame('main()', []),
      frame(fnLabel, [['left', '0'], ['right', String(right)], ['s[right]', `'${chars[right]}'`]])
    ],
    cells: currentCellStates(left, right),
    left, right, matches: 0, n, result: null
  });
  ...
  while (left < right) {
    steps.push({
      ...
      vars: [
        frame('main()', []),
        frame(fnLabel, [['left', String(left)], ['right', String(right)], ['s[left]', `'${chars[left]}'`], ['s[right]', `'${chars[right]}'`]])
      ],
      cells: currentCellStates(left, right),
      left, right, matches, n, result: null
    });

    const isMatch = chars[left] === chars[right];
    steps.push({
      badge: `if (s[${left}] != s[${right}]) → comparing '${chars[left]}' and '${chars[right]}' → ${isMatch ? 'MATCH!' : 'MISMATCH!'}`,
      code: 'c_check',
      vars: [
        frame('main()', []),
        frame(fnLabel, [['left', String(left)], ['right', String(right)], ['s[left]', `'${chars[left]}'`], ['s[right]', `'${chars[right]}'`]])
    ...
    if (!isMatch) {
      isPal = false;
      mismatchPair = [left, right];
      steps.push({
        badge: `return false → Mismatch found at indices ${left} ('${chars[left]}') and ${right} ('${chars[right]}') → NOT a palindrome`,
        code: 'c_mismatch',
        vars: [
          frame('main()', []),
          frame(fnLabel, [['return', 'false'], ['status', 'MISMATCH']])
        ],
        cells: currentCellStates(left, right),
        left, right, matches, n, result: false
      });
      break;
    }
    ...
    matches++;
    matchedSet.add(left);
    matchedSet.add(right);

    left++;
    right--;
    steps.push({
      badge: `left++ (${left}), right-- (${right}) → pointers moved inward`,
      code: 'c_inc',
      vars: [
        frame('main()', []),
  ...
  if (isPal) {
    if (left === right) matchedSet.add(left);
    steps.push({
      badge: `return true → All character pairs matched! String "${initialStr}" IS a palindrome`,
      code: 'c_return_true',
      vars: [
        frame('main()', []),
        frame(fnLabel, [['return', 'true'], ['status', 'PALINDROME']])
      ],
      cells: chars.map((v, idx) => ({ idx, val: v, state: 'matched' })),
      left: -1, right: -1, matches, n, result: true
    });
  }

  steps.push({
    badge: `Palindrome Check Complete! Result: "${initialStr}" is ${isPal ? 'a PALINDROME (true)' : 'NOT a palindrome (false)'}`,
    code: 'c_done',
    vars: [
      frame('main()', []),
      frame(fnLabel, [['result', String(isPal)]])
    ],
    cells: chars.map((v, idx) => {
      if (mismatchPair.includes(idx)) return { idx, val: v, state: 'mismatch' };
      if (matchedSet.has(idx)) return { idx, val: v, state: 'matched' };
      return { idx, val: v, state: 'normal' };
    }),
    left: -1, right: -1, matches, n, result: isPal
  });

  return steps;
}

const inpText = ref('RADAR');
const lang = ref('java');
const speed = ref(650);
const si = ref(0);
...
const stepsData = reactive({ steps: buildSteps('RADAR') });
...
function cellLabel(cell) {
  ...
  if (cell.state === 'matched') return 'matched';
  if (cell.state === 'mismatch') return 'mismatch';
  return '';
}
      ...
      <div class="row-main">
        <div class="ll-root">
          ...
          <div class="ll-toolbar">
            <label>Text</label>
            <input type="text" v-model="inpText" style="width: 220px;" class="ll-text-input" placeholder="e.g. RADAR" />
          ...
          <div class="ll-main" ref="mainRef">
            ...
            <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
              <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
                <div class="ll-perm-area">
                  <div class="ll-ptrs">
                    ...
                    <div class="ll-ptr-chip">Result = <b :class="s.result === true ? 'll-c-green' : (s.result === false ? 'll-c-red' : 'll-c-orange')">{{ s.result === null ? 'Checking...' : String(s.result) }}</b></div>
                  </div>
                      ...
                      <div class="ll-arr-cell-wrap">
                        ...
                        <div
                          ...
                          :class="{
                            ...
                            'll-box-matched': cell.state === 'matched',
                            'll-box-mismatch': cell.state === 'mismatch'
                          }"
                        >
                          {{ cell.val }}
                        </div>
              ...
              <!-- Color Legend -->
              <div class="ll-legend">
                <span class="ll-leg"><span class="ll-legdot ll-legdot-existing"></span>Unchecked</span>
                <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>Comparing Pair</span>
                <span class="ll-leg"><span class="ll-legdot ll-legdot-matched"></span>Matched Pair</span>
              ...
              <div class="ll-code-panel">
                ...
                <div v-else class="ll-info-scroll">
                  ...
                  <table class="ll-complexity-table">
                    ...
                    <tbody>
                      <tr><td>Best Case</td><td>O(1)</td><td>Mismatch at first and last character s[0] != s[n-1].</td></tr>
                      <tr><td>Average Case</td><td>O(n)</td><td>Scans through up to n/2 character comparisons.</td></tr>
                      <tr><td>Worst Case</td><td>O(n)</td><td>Full string scan when string is a palindrome.</td></tr>
                      <tr><td>Space</td><td>O(1)</td><td>Two-pointer scan using constant auxiliary space.</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    Key observation: A <b>Palindrome</b> reads the same forward and backward. The two-pointer approach compares <code>s[left]</code> and <code>s[right]</code> while moving pointers inward.
                  </p>
                </div>
              </div>
            </div>
...
.ll-box-mismatch { border-color: #ef4444 !important; background: #fef2f2 !important; color: #991b1b !important; box-shadow: 0 0 0 3px rgba(239, 68, 68, 0.25) !important; transform: scale(1.05); }
