...
defineProps({
  topic: { type: String, default: 'Longest Common Substring' },
  subTopic: { type: String, default: '2D Dynamic Programming Contiguous Substring Grid' }
});

const CODES = {
  java: [
    ['', 'class LongestCommonSubstring {'],
    ['c_entry', '    int longestCommonSubstring(String s1, String s2) {'],
    ['c_m', '        int m = s1.length();'],
    ['c_n', '        int n = s2.length();'],
    ['c_dp_init', '        int[][] dp = new int[m + 1][n + 1];'],
    ['c_vars', '        int maxLen = 0; int endIndex = -1;'],
    ['', ''],
    ...
    ['c_max_check', '                    if (dp[i][j] > maxLen) {'],
    ['c_max_update', '                        maxLen = dp[i][j]; endIndex = i - 1;'],
    ['', '                    }'],
    ['', '                } else {'],
    ['c_mismatch_reset', '                    dp[i][j] = 0;'],
    ['', '                }'],
    ['', '            }'],
    ['', '        }'],
    ['c_return', '        return maxLen;'],
    ['c_done', '    }'],
    ['', '}']
  ],
  c: [
    ['c_entry', 'int longestCommonSubstring(const char* s1, const char* s2) {'],
    ['c_m', '    int m = strlen(s1);'],
    ['c_n', '    int n = strlen(s2);'],
  ...
  python: [
    ['', 'class LongestCommonSubstring:'],
    ['c_entry', '    def longestCommonSubstring(self, s1: str, s2: str) -> int:'],
    ['c_m', '        m = len(s1)'],
    ['c_n', '        n = len(s2)'],
    ['c_dp_init', '        dp = [[0] * (n + 1) for _ in range(m + 1)]'],
    ['c_vars', '        max_len = 0; end_index = -1'],
    ['c_outer_loop', '        for i in range(1, m + 1):'],
    ['c_inner_loop', '            for j in range(1, n + 1):'],
    ['c_char_check', '                if s1[i - 1] == s2[j - 1]:'],
    ['c_match_calc', '                    dp[i][j] = dp[i - 1][j - 1] + 1'],
    ['c_max_check', '                    if dp[i][j] > max_len:'],
    ['c_max_update', '                        max_len = dp[i][j]; end_index = i - 1'],
    ['', '                else:'],
    ['c_mismatch_reset', '                    dp[i][j] = 0'],
    ['c_return', '        return max_len'],
    ['c_done', '']
  ],
  javascript: [
    ['', 'class LongestCommonSubstring {'],
    ['c_entry', '  longestCommonSubstring(s1, s2) {'],
    ['c_m', '    const m = s1.length;'],
    ['c_n', '    const n = s2.length;'],
    ['c_dp_init', '    const dp = Array.from({ length: m + 1 }, () => new Array(n + 1).fill(0));'],
    ['c_vars', '    let maxLen = 0; let endIndex = -1;'],
...
function buildSteps(initialS1, initialS2) {
  ...
  const m = s1.length;
  const n = s2.length;
  const fnLabel = 'longestCommonSubstring(s1, s2)';
  ...
  function getGridCells(curI = -1, curJ = -1, isMatch = false, isMismatch = false) {
    const grid = [];
    for (let r = 0; r <= m; r++) {
      const rowCells = [];
      for (let c = 0; c <= n; c++) {
        let state = 'normal';
        if (maxLen > 0 && r === maxI && c === maxJ) {
          state = 'max_cell';
        } else if (r === curI && c === curJ) {
          if (isMatch) state = 'match';
          else if (isMismatch) state = 'mismatch';
          else state = 'active';
        } else if (curI > 0 && curJ > 0 && isMatch && r === curI - 1 && c === curJ - 1) {
          state = 'parent_diag';
        }
  ...
  function getS1Cells(curI = -1) {
    return s1.split('').map((val, idx) => {
      let state = 'normal';
      if (maxLen > 0 && endIndex >= 0 && idx >= endIndex - maxLen + 1 && idx <= endIndex) state = 'substring_match';
      else if (idx === curI - 1) state = 'active';
  ...
  function getS2Cells(curJ = -1) {
    ...
    return s2.split('').map((val, idx) => {
      let state = 'normal';
      if (maxLen > 0 && endJ >= 0 && idx >= endJ - maxLen + 1 && idx <= endJ) state = 'substring_match';
      else if (idx === curJ - 1) state = 'active';
      return { idx, val, state };
    });
  }

  function getSubstrVal() {
    return (maxLen > 0 && endIndex >= 0) ? s1.substring(endIndex - maxLen + 1, endIndex + 1) : '';
  }

  // 1. Entry
  steps.push({
    badge: `longestCommonSubstring called with s1="${s1}", s2="${s2}"`,
    code: 'c_entry',
    vars: [
      frame('main()', []),
    ...
    i: -1, j: -1, s1, s2, m, n, maxLen: 0, substr: ''
  });
  ...
  steps.push({
    badge: `int[][] dp = new int[${m + 1}][${n + 1}] → Initialized 2D DP matrix with base 0s`,
    code: 'c_dp_init',
    vars: [
      frame('main()', []),
    ...
    grid: getGridCells(),
    s1Cells: getS1Cells(), s2Cells: getS2Cells(),
    i: 0, j: 0, s1, s2, m, n, maxLen: 0, substr: ''
  });

  // 5. int maxLen = 0, endIndex = -1
  steps.push({
    badge: `int maxLen = 0; int endIndex = -1; → Initialized tracking variables`,
    code: 'c_vars',
    vars: [
      frame('main()', []),
      frame(fnLabel, [['maxLen', '0'], ['endIndex', '-1']])
    ],
    grid: getGridCells(),
    s1Cells: getS1Cells(), s2Cells: getS2Cells(),
    i: 0, j: 0, s1, s2, m, n, maxLen: 0, substr: ''
  ...
  // 6. Nested Loops i = 1..m and j = 1..n
  for (let i = 1; i <= m; i++) {
    steps.push({
      ...
      i, j: 0, s1, s2, m, n, maxLen, substr: getSubstrVal()
    });
    ...
    for (let j = 1; j <= n; j++) {
      steps.push({
        ...
        i, j, s1, s2, m, n, maxLen, substr: getSubstrVal()
      });
      ...
      if (isMatch) {
        ...
        const isNewMax = dp[i][j] > maxLen;
        steps.push({
          badge: `if (dp[${i}][${j}] > maxLen) → ${dp[i][j]} > ${maxLen} → ${isNewMax ? 'NEW MAX SUBSTRING!' : 'NO UPDATE'}`,
          code: 'c_max_check',
          vars: [
            frame('main()', []),
            frame(fnLabel, [['dp[i][j]', String(dp[i][j])], ['maxLen', String(maxLen)], ['isNewMax', String(isNewMax)]])
          ],
          grid: getGridCells(i, j, true, false),
          s1Cells: getS1Cells(i), s2Cells: getS2Cells(j),
          i, j, s1, s2, m, n, maxLen, substr: getSubstrVal()
        ...
        if (isNewMax) {
          ...
          steps.push({
            badge: `maxLen = ${maxLen}, endIndex = ${endIndex} → New longest common substring: "${getSubstrVal()}"`,
            code: 'c_max_update',
            vars: [
              frame('main()', []),
              frame(fnLabel, [['maxLen', String(maxLen)], ['endIndex', String(endIndex)], ['substring', `"${getSubstrVal()}"`]])
            ],
            grid: getGridCells(i, j, true, false),
            s1Cells: getS1Cells(i), s2Cells: getS2Cells(j),
            i, j, s1, s2, m, n, maxLen, substr: getSubstrVal()
        ...
        dp[i][j] = 0;
        steps.push({
          badge: `dp[${i}][${j}] = 0 → contiguous match reset to 0`,
          code: 'c_mismatch_reset',
          vars: [
            frame('main()', []),
            frame(fnLabel, [['i', String(i)], ['j', String(j)], ['dp[i][j]', '0']])
          ],
  ...
  // 7. Return maxLen
  const finalSubstr = getSubstrVal();
  steps.push({
    badge: `return maxLen → Longest Common Substring: "${finalSubstr}" (length = ${maxLen})`,
    code: 'c_return',
    vars: [
      frame('main()', []),
      frame(fnLabel, [['return', String(maxLen)], ['substring', `"${finalSubstr}"`]])
    ],
    grid: getGridCells(),
    s1Cells: getS1Cells(), s2Cells: getS2Cells(),
    i: m, j: n, s1, s2, m, n, maxLen, substr: finalSubstr
  });

  steps.push({
    badge: `Longest Common Substring Complete! Substring: "${finalSubstr}" (length = ${maxLen})`,
    code: 'c_done',
    vars: [
      frame('main()', []),
      frame(fnLabel, [['result_maxLen', String(maxLen)], ['result_substr', `"${finalSubstr}"`]])
    ],
    grid: getGridCells(),
    s1Cells: getS1Cells(), s2Cells: getS2Cells(),
    i: m, j: n, s1, s2, m, n, maxLen, substr: finalSubstr
...
const inpS1 = ref('abcdef');
const inpS2 = ref('abfdef');
const lang = ref('java');
const speed = ref(650);
const si = ref(0);
...
const stepsData = reactive({ steps: buildSteps('abcdef', 'abfdef') });
      ...
      <div class="row-main">
        <div class="ll-root">
          ...
          <div class="ll-toolbar">
            ...
            <input type="text" v-model="inpS1" style="width: 110px;" class="ll-text-input" placeholder="e.g. abcdef" />

            <label>String S2</label>
            <input type="text" v-model="inpS2" style="width: 110px;" class="ll-text-input" placeholder="e.g. abfdef" />
          ...
          <div class="ll-main" ref="mainRef">
            ...
            <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
              <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
                <div class="ll-perm-area">
                  <div class="ll-ptrs">
                    ...
                    <div class="ll-ptr-chip">Common Substring = <b class="ll-c-green">"{{ s.substr }}"</b></div>
                  </div>
                    ...
                    <table class="ll-dp-grid">
                      <thead>
                        <tr>
                          ...
                          <th v-for="(cCell, cIdx) in s.s2Cells" :key="'col-' + cIdx" class="ll-dp-header" :class="{ 'll-header-active': cCell.state === 'active', 'll-header-match': cCell.state === 'substring_match' }">
                            {{ cCell.val }}
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr v-for="(row, rIdx) in s.grid" :key="'row-' + rIdx">
                          <td class="ll-dp-header" :class="{ 'll-header-active': rIdx > 0 && s.s1Cells[rIdx - 1] && s.s1Cells[rIdx - 1].state === 'active', 'll-header-match': rIdx > 0 && s.s1Cells[rIdx - 1] && s.s1Cells[rIdx - 1].state === 'substring_match' }">
                            {{ rIdx === 0 ? '""' : s.s1[rIdx - 1] }}
                          </td>
                          <td v-for="(cell, cIdx) in row" :key="'cell-' + rIdx + '-' + cIdx" class="ll-dp-cell" :class="{ 'll-cell-active': cell.state === 'active', 'll-cell-match': cell.state === 'match', 'll-cell-mismatch': cell.state === 'mismatch', 'll-cell-diag': cell.state === 'parent_diag', 'll-cell-max': cell.state === 'max_cell' }">
                            {{ cell.val }}
                          </td>
                        </tr>
                      </tbody>
              ...
              <div class="ll-legend">
                ...
                <span class="ll-leg"><span class="ll-legdot ll-legdot-mismatch"></span>Mismatch (0)</span>
                <span class="ll-leg"><span class="ll-legdot ll-legdot-max"></span>Max Length Cell</span>
              </div>
                ...
                <div v-else class="ll-info-scroll">
                  ...
                  <table class="ll-complexity-table">
                    ...
                    <tbody>
                      ...
                      <tr><td>Space Complexity</td><td>O(m &times; n)</td><td>Stores 2D dynamic programming table (or O(n) space-optimized).</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    Key observation: <b>Longest Common Substring</b> requires contiguous matching! Hence, <code>dp[i][j] = dp[i-1][j-1] + 1</code> on match, but resets to <code>dp[i][j] = 0</code> on mismatch!
                  </p>
                </div>
              </div>
            </div>
...
.ll-header-match { background: #dcfce7 !important; color: #15803d !important; border-color: #22c55e !important; }
...
.ll-cell-mismatch { background: #f8fafc !important; border: 1px solid #cbd5e1 !important; color: #94a3b8 !important; }
.ll-cell-diag { background: #f0fdf4 !important; border: 1.5px dashed #22c55e !important; }
.ll-cell-max { background: #fef3c7 !important; border: 2px solid #f59e0b !important; color: #92400e !important; font-size: 14px; font-weight: 900; }
...
.ll-legdot-mismatch { background: #f8fafc; border: 1.5px solid #cbd5e1; }
.ll-legdot-max { background: #fef3c7; border: 1.5px solid #f59e0b; }
