...
defineProps({
  topic: { type: String, default: 'Linear Search Algorithm' },
...
const CODES = {
  java: [
    ['', 'class LinearSearch {'],
    ['c_entry', '    int linearSearch(int[] arr, int target) {'],
    ['c_n', '        int n = arr.length;'],
    ['c_loop', '        for (int i = 0; i < n; i++) {'],
    ['c_check', '            if (arr[i] == target) {'],
    ['c_found', '                return i;'],
    ['', '            }'],
    ['', '        }'],
    ['c_notfound', '        return -1;'],
  ...
  c: [
    ['c_entry', 'int linearSearch(int arr[], int n, int target) {'],
    ['c_loop', '    for (int i = 0; i < n; i++) {'],
    ['c_check', '        if (arr[i] == target) {'],
    ['c_found', '            return i;'],
    ['', '        }'],
  ...
  cpp: [
    ['', 'class LinearSearch {'],
    ['', 'public:'],
    ['c_entry', '    int linearSearch(const vector<int>& arr, int target) {'],
    ['c_n', '        int n = arr.size();'],
    ['c_loop', '        for (int i = 0; i < n; i++) {'],
    ['c_check', '            if (arr[i] == target) {'],
    ['c_found', '                return i;'],
  ...
  python: [
    ['', 'class LinearSearch:'],
    ['c_entry', '    def linear_search(self, arr, target):'],
    ['c_n', '        n = len(arr)'],
  ...
  javascript: [
    ['', 'class LinearSearch {'],
    ['c_entry', '  linearSearch(arr, target) {'],
...
function buildSteps(values, searchTarget) {
  ...
  const fnLabel = 'linearSearch(arr, target)';

  // 1. Function entry
  steps.push({
    badge: `linearSearch called with target = ${searchTarget} across array of size ${n}`,
    code: 'c_entry',
    vars: [
  ...
  steps.push({
    ...
    vars: [
      frame('main()', [['target', String(searchTarget)]]),
      frame(fnLabel, [['n', String(n)], ['target', String(searchTarget)]])
    ],
    cells: values.map((val, idx) => ({ idx, val, state: 'normal' })),
    i: -1,
    size: n,
  ...
  for (let i = 0; i < n; i++) {
    ...
    steps.push({
      badge: `for (i = ${i}; i < ${n}; i++) → condition i < n holds, evaluating index ${i}`,
      code: 'c_loop',
      vars: [
        frame('main()', [['target', String(searchTarget)]]),
        frame(fnLabel, [['n', String(n)], ['target', String(searchTarget)], ['i', String(i)]])
      ],
      cells: values.map((v, idx) => ({
        idx,
        val: v,
        state: idx === i ? 'cur' : (idx < i ? 'eliminated' : 'normal')
      })),
      i,
      size: n,
      target: searchTarget
    ...
    steps.push({
      badge: `if (arr[${i}] == ${searchTarget}) → checking ${val} == ${searchTarget}`,
      code: 'c_check',
      vars: [
        frame('main()', [['target', String(searchTarget)]]),
        frame(fnLabel, [['n', String(n)], ['target', String(searchTarget)], ['i', String(i)], ['arr[i]', String(val)]])
      ],
      cells: values.map((v, idx) => ({
        idx,
        val: v,
    ...
    if (val === searchTarget) {
      steps.push({
        badge: `arr[${i}] (${val}) == ${searchTarget} is TRUE → return ${i} (Match found!)`,
        code: 'c_found',
        vars: [
          frame('main()', [['target', String(searchTarget)]]),
          frame(fnLabel, [['n', String(n)], ['target', String(searchTarget)], ['i', String(i)], ['return', String(i)]])
        ],
        cells: values.map((v, idx) => ({
          idx,
          val: v,
          state: idx === i ? 'found' : (idx < i ? 'eliminated' : 'normal')
        })),
        i,
        size: n,
        target: searchTarget
  ...
  steps.push({
    badge: `for loop completed (i = ${n} < ${n} is FALSE) → exit loop`,
    code: 'c_loop',
    vars: [
      frame('main()', [['target', String(searchTarget)]]),
      frame(fnLabel, [['n', String(n)], ['target', String(searchTarget)], ['i', String(n)]])
    ],
  ...
  steps.push({
    badge: `return -1 → target ${searchTarget} not found in array`,
    code: 'c_notfound',
    invalid: true,
    vars: [
      frame('main()', [['target', String(searchTarget)]]),
      frame(fnLabel, [['n', String(n)], ['target', String(searchTarget)], ['return', '-1']])
    ],
    cells: values.map((v, idx) => ({ idx, val: v, state: 'eliminated' })),
    i: -1,
    size: n,
...
const inpElems = ref('12 25 34 42 58 69 81');
...
const initialArray = [12, 25, 34, 42, 58, 69, 81];
...
function cellLabel(cell) {
  if (s.value.i === cell.idx) return 'i \u2192';
  return '';
}
      ...
      <div class="row-main">
        <div class="ll-root">
          ...
          <div class="ll-toolbar">
            ...
            <input type="text" v-model="inpElems" style="width: 180px;" class="ll-text-input" placeholder="e.g. 10 20 30 40" />
          ...
          <div class="ll-main" ref="mainRef">
            ...
            <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
              <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
                <div class="ll-perm-area">
                  <div class="ll-ptrs">
                    ...
                    <div class="ll-ptr-chip">i = <b class="ll-c-orange">{{ s.i >= 0 ? s.i : 'N/A' }}</b></div>
                    ...
                    <template v-for="cell in s.cells" :key="cell.idx">
                      <div class="ll-arr-cell-wrap">
                        <div class="ll-ptr-tag-wrap">
                          <span v-if="cell.idx === s.i" class="ll-ptr-lbl ll-lbl-orange">i</span>
                        </div>
                        <div
                          class="ll-arr-box"
                          :class="{
              ...
              <div class="ll-legend">
                ...
                <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>Inspecting / Comparing (i)</span>
                <span class="ll-leg"><span class="ll-legdot ll-legdot-new"></span>Target Found</span>
                <span class="ll-leg"><span class="ll-legdot" style="background:var(--surface2); border:1.5px dashed var(--border2);"></span>Checked / Not Match</span>
              </div>
              ...
              <div class="ll-code-panel">
                ...
                <div v-else class="ll-info-scroll">
                  ...
                  <table class="ll-complexity-table">
                    ...
                    <tbody>
                      <tr><td>Best Case</td><td>O(1)</td><td>Target element is located at index 0 (first comparison).</td></tr>
                      <tr><td>Average Case</td><td>O(n)</td><td>Target element is near the middle on average (n/2 comparisons).</td></tr>
                      <tr><td>Worst Case</td><td>O(n)</td><td>Target element is at index n-1 or absent (n comparisons).</td></tr>
                      <tr><td>Space</td><td>O(1)</td><td>Operates in-place with a single loop variable <code>i</code>.</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    Key observation: Linear search requires no preprocessing or sorting. It checks elements one by one from index 0 to n-1 until a match is found.
                  </p>
                  <h3>Advantages</h3>
                  <p>Simple to implement, works on unsorted or unordered arrays and linked lists alike, and requires no auxiliary memory.</p>
                  <h3>Limitations</h3>
                  <p>Inefficient for large arrays since time scales linearly with dataset size O(n).</p>
                </div>
              </div>
            </div>
          </div>
