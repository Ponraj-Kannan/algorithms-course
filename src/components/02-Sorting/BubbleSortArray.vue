...
defineProps({
  topic: { type: String, default: 'Bubble Sort Algorithm' },
...
const CODES = {
  java: [
    ['', 'class BubbleSort {'],
    ['c_entry', '    void bubbleSort(int[] arr) {'],
    ['c_n', '        int n = arr.length;'],
    ['c_outer', '        for (int i = 0; i < n - 1; i++) {'],
    ['c_swapped_init', '            boolean swapped = false;'],
    ['c_inner', '            for (int j = 0; j < n - i - 1; j++) {'],
    ['c_check', '                if (arr[j] > arr[j + 1]) {'],
    ['c_swap_temp', '                    int temp = arr[j];'],
    ['c_swap_w1', '                    arr[j] = arr[j + 1];'],
    ['c_swap_w2', '                    arr[j + 1] = temp;'],
    ['c_swapped_set', '                    swapped = true;'],
    ['', '                }'],
    ['', '            }'],
    ['c_break_check', '            if (!swapped) break;'],
    ['', '        }'],
    ['c_done', '    }'],
    ['', '}']
  ],
  c: [
    ['c_entry', 'void bubbleSort(int arr[], int n) {'],
    ['c_outer', '    for (int i = 0; i < n - 1; i++) {'],
    ['c_swapped_init', '        int swapped = 0;'],
    ['c_inner', '        for (int j = 0; j < n - i - 1; j++) {'],
    ['c_check', '            if (arr[j] > arr[j + 1]) {'],
    ['c_swap_temp', '                int temp = arr[j];'],
    ['c_swap_w1', '                arr[j] = arr[j + 1];'],
    ['c_swap_w2', '                arr[j + 1] = temp;'],
    ['c_swapped_set', '                swapped = 1;'],
    ['', '            }'],
    ['', '        }'],
    ['c_break_check', '        if (!swapped) break;'],
    ['', '    }'],
  ...
  cpp: [
    ['', 'class BubbleSort {'],
    ['', 'public:'],
    ['c_entry', '    void bubbleSort(vector<int>& arr) {'],
    ['c_n', '        int n = arr.size();'],
    ['c_outer', '        for (int i = 0; i < n - 1; i++) {'],
    ['c_swapped_init', '            bool swapped = false;'],
    ['c_inner', '            for (int j = 0; j < n - i - 1; j++) {'],
    ['c_check', '                if (arr[j] > arr[j + 1]) {'],
    ['c_swap_temp', '                    int temp = arr[j];'],
    ['c_swap_w1', '                    arr[j] = arr[j + 1];'],
  ...
  python: [
    ['', 'class BubbleSort:'],
    ['c_entry', '    def bubble_sort(self, arr):'],
    ['c_n', '        n = len(arr)'],
    ['c_outer', '        for i in range(n - 1):'],
    ['c_swapped_init', '            swapped = False'],
    ['c_inner', '            for j in range(n - i - 1):'],
    ['c_check', '                if arr[j] > arr[j + 1]:'],
    ['c_swap_temp', '                    temp = arr[j]'],
    ['c_swap_w1', '                    arr[j] = arr[j + 1]'],
    ['c_swap_w2', '                    arr[j + 1] = temp'],
    ['c_swapped_set', '                    swapped = True'],
    ['c_break_check', '            if not swapped:'],
    ['c_done', '                break']
  ],
  javascript: [
    ['', 'class BubbleSort {'],
    ['c_entry', '  bubbleSort(arr) {'],
    ['c_n', '    const n = arr.length;'],
    ['c_outer', '    for (let i = 0; i < n - 1; i++) {'],
    ['c_swapped_init', '      let swapped = false;'],
    ['c_inner', '      for (let j = 0; j < n - i - 1; j++) {'],
    ['c_check', '        if (arr[j] > arr[j + 1]) {'],
    ['c_swap_temp', '          let temp = arr[j];'],
    ['c_swap_w1', '          arr[j] = arr[j + 1];'],
    ['c_swap_w2', '          arr[j + 1] = temp;'],
    ['c_swapped_set', '          swapped = true;'],
    ['', '        }'],
...
function buildSteps(initialValues) {
  ...
  const n = arr.length;
  const fnLabel = 'bubbleSort(arr)';

  function currentCellStates(i, j, sortedSet, curPair = [], swapPair = []) {
    return arr.map((val, idx) => {
      if (sortedSet.has(idx)) return { idx, val, state: 'sorted' };
      if (swapPair.includes(idx)) return { idx, val, state: 'swap' };
      if (curPair.includes(idx)) return { idx, val, state: 'cur' };
  ...
  // 1. Function entry
  steps.push({
    badge: `bubbleSort called with array of size ${n}`,
    code: 'c_entry',
    vars: [
      frame('main()', []),
  ...
  for (let i = 0; i < n - 1; i++) {
    steps.push({
      badge: `for (i = ${i}; i < ${n - 1}; i++) → Pass ${i + 1}`,
      code: 'c_outer',
      vars: [
        frame('main()', []),
    ...
    let swapped = false;
    steps.push({
      badge: `boolean swapped = false → initialize swapped flag for Pass ${i + 1}`,
      code: 'c_swapped_init',
      vars: [
        frame('main()', []),
    ...
    for (let j = 0; j < n - i - 1; j++) {
      ...
      // Inner loop header
      steps.push({
        badge: `for (j = ${j}; j < ${n - i - 1}; j++) → compare elements at index ${j} and ${j + 1}`,
        code: 'c_inner',
        vars: [
          frame('main()', []),
      ...
      // Check condition
      steps.push({
        badge: `if (arr[${j}] > arr[${j + 1}]) → checking ${arr[j]} > ${arr[j + 1]}`,
        code: 'c_check',
        vars: [
          frame('main()', []),
      ...
      if (arr[j] > arr[j + 1]) {
        ...
        const temp = arr[j];
        steps.push({
          badge: `int temp = arr[${j}] → temp = ${temp} (${arr[j]} > ${arr[j + 1]} is TRUE)`,
          code: 'c_swap_temp',
          vars: [
            frame('main()', []),
        ...
        arr[j] = arr[j + 1];
        steps.push({
          badge: `arr[${j}] = arr[${j + 1}] → arr[${j}] set to ${arr[j]}`,
          code: 'c_swap_w1',
          vars: [
            frame('main()', []),
        ...
        swaps++;
        steps.push({
          badge: `arr[${j + 1}] = temp → arr[${j + 1}] set to ${temp}`,
          code: 'c_swap_w2',
          vars: [
            frame('main()', []),
        ...
        swapped = true;
        steps.push({
          badge: `swapped = true → swap completed between index ${j} and ${j + 1}`,
          code: 'c_swapped_set',
          vars: [
            frame('main()', []),
    ...
    // Element at n - i - 1 is now in its final position
    sortedSet.add(n - i - 1);
    steps.push({
      badge: `Pass ${i + 1} finished → element ${arr[n - i - 1]} at index ${n - i - 1} is now in sorted position`,
      code: 'c_outer',
      vars: [
        frame('main()', []),
    ...
    steps.push({
      badge: `if (!swapped) → swapped is ${swapped}`,
      code: 'c_break_check',
      vars: [
        frame('main()', []),
    ...
    if (!swapped) {
      steps.push({
        badge: 'swapped is false → array is already sorted! Breaking early',
        code: 'c_break_check',
        vars: [
          frame('main()', []),
  ...
  // Mark all elements sorted at end
  for (let k = 0; k < n; k++) sortedSet.add(k);

  steps.push({
    badge: `Bubble Sort Complete! Total comparisons: ${comparisons}, Total swaps: ${swaps}`,
    code: 'c_done',
    vars: [
      frame('main()', []),
...
const inpElems = ref('45 12 89 34 23 70 10 58');
const lang = ref('java');
const speed = ref(650);
const si = ref(0);
...
const initialArray = [45, 12, 89, 34, 23, 70, 10, 58];
...
function cellLabel(cell) {
  ...
  if (s.value.j + 1 === cell.idx && s.value.j >= 0) tags.push('j+1');
      ...
      <div class="row-main">
        <div class="ll-root">
          ...
          <div class="ll-toolbar">
            ...
            <input type="text" v-model="inpElems" style="width: 220px;" class="ll-text-input" placeholder="e.g. 45 12 89 34 23" />
          ...
          <div class="ll-main" ref="mainRef">
            ...
            <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
              ...
              <div class="ll-legend">
                ...
                <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>Comparing (j, j+1)</span>
              ...
              <div class="ll-code-panel">
                ...
                <div v-else class="ll-info-scroll">
                  ...
                  <table class="ll-complexity-table">
                    ...
                    <tbody>
                      <tr><td>Best Case</td><td>O(n)</td><td>Array is already sorted; detected in 1 pass when <code>swapped == false</code>.</td></tr>
                      <tr><td>Average Case</td><td>O(n²)</td><td>Requires n(n-1)/4 comparisons and swaps on average.</td></tr>
                      <tr><td>Worst Case</td><td>O(n²)</td><td>Array is sorted in reverse order; requires n(n-1)/2 swaps.</td></tr>
                      <tr><td>Space</td><td>O(1)</td><td>In-place sorting algorithm using constant extra memory.</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    Key observation: After pass <code>i</code>, the largest unsorted element "bubbles up" to its final correct position at index <code>n - i - 1</code>.
                  </p>
                  <h3>Advantages</h3>
                  <p>Simple to understand and implement. Detects already sorted arrays in O(n) time using the <code>swapped</code> flag.</p>
                  <h3>Limitations</h3>
                  <p>Inefficient for large arrays due to quadratic O(n²) time complexity.</p>
                </div>
              </div>
            </div>
          </div>
